...
export const DataTable = (props: DataTableProps) => {
  ...
  return (
    <div
      className={cn(
        'relative flex min-h-[580px] flex-col overflow-hidden rounded-md bg-white',
        props.className,
      )}
    >
      {status ? (
                      ...
                      {header.isPlaceholder ? null : (
                        ...
                        >
                          ...
                          <div>
                            {{
                              asc: (
                                <ArrowDownAZ className='h-5 w-5 text-stone-600' />
                              ),
                              desc: (
                                <ArrowUpZA className='h-5 w-5 text-stone-600' />
                              ),
      ...
      {!status && (
        <div className='flex flex-col items-center justify-center px-4 py-6 md:flex-row md:justify-between md:px-6'>
          <div>
            ...
            <b>{Math.round(pageCount / perPage)}</b> of <b>{pageCount}</b> data
          </div>
